...
type Log struct {
	...
	SystemName       string `json:"system_name" gorm:"default:0"`
...
func RecordConsumeLog(ctx context.Context, userId int, SystemName string, channelId int, promptTokens int, completionTokens int, modelName string, tokenName string, quota int, content string) {
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, systemName string, groupName string) (logs []*Log, err error) {
	var tx *gorm.DB
	...
	if systemName != "" {
		tx = tx.Where("system_name = ?", systemName)
	}
	if startTimestamp != 0 {
