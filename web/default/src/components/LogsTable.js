...
const LogsTable = () => {
  ...
  const { username,system_name,group_name, token_name, model_name, start_timestamp, end_timestamp, channel } = inputs;
  ...
  const getLogSelfStat = async () => {
    ...
    let res = await API.get(`/api/log/self/stat?type=${logType}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&system_name=${system_name}&group_name=${group_name}`);
  ...
  const loadLogs = async (startIdx) => {
    ...
    let localEndTimestamp = Date.parse(end_timestamp) / 1000;
    if (isAdminUser) {
      url = `/api/log/?p=${startIdx}&type=${logType}&username=${username}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&channel=${channel}&system_name=${system_name}&group_name=${group_name}`;
    } else {
      url = `/api/log/self/?p=${startIdx}&type=${logType}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&system_name=${system_name}&group_name=${group_name}`;
    }
            ...
            isAdminUser && <>
              <Form.Group>
                ...
                <Form.Input fluid label={'用户名称'} width={3} value={username}
                            ...
                            onChange={handleInputChange} />
                <Form.Input fluid label={'平台用户'} width={3} value={system_name}
                            placeholder={'可选值'} name='system_name'
                            onChange={handleInputChange} />
                <Form.Dropdown
                    label='分组'
                    placeholder={''}
              ...
              .map((log, idx) => {
                ...
                return (
                  <Table.Row key={log.id}>
                    ...
                    <Table.Cell>{log.system_name ? <Label>{log.system_name}</Label> : ''}</Table.Cell>
                    <Table.Cell>{log.RequestCount ? <Label>{log.RequestCount}</Label> : ''}</Table.Cell>
